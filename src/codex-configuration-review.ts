...
export function configurationReviewContext(target: IntegrationTarget, inputs?: readonly FileSnapshot[], proposed?: CodexIntegrationJournal): { baseSource?: string; trackedPaths: string[]; managedHookPaths: string[] } {
  ...
  const hookPaths = (journal: CodexIntegrationJournal) => {
    const hook = journal.interruptHook;
    return ["command", "type", "timeout", "async"].map(key => configurationPathName(["hooks", "Interrupt", hook.groupIndex, "hooks", 0, key]))
      .concat(configurationPathName(["hooks", "state", hook.stateKey, "trusted_hash"]));
  };
  try {
    ...
    if (journal && journal.configPath === target.configPath && journal.version === 10 && journal.active) {
      ...
      managedHookPaths.push(...hookPaths(journal));
    }
  } catch { /* Unreadable ownership is not evidence of ownership. */ }
  if (proposed && proposed.configPath === target.configPath) managedHookPaths.push(...hookPaths(proposed));
  if (target.kind === "base") return { trackedPaths, managedHookPaths };
  const path = join(target.codexHome, "config.toml");
  const base = inputs?.find(input => input.path === path) ?? snapshotFile(path);
  return { trackedPaths, managedHookPaths, baseSource: base.data?.toString("utf8") ?? "" };
}
...
function groupFor(path: string, managedHook: boolean): ConfigurationReviewGroup["id"] {
  ...
  if (path.startsWith("hooks.")) return managedHook ? "interrupt" : "integrations";
...
export function withConfigurationReview(preview: CodexRepairPreview, target: IntegrationTarget, source: string, options: { baseSource?: string; proposedBaseSource?: string; trackedPaths?: readonly string[]; managedHookPaths?: readonly string[] } = {}): CodexRepairPreview {
  const layers = [
  ...
  for (const identity of paths) {
    ...
    const groupId = sectionChoice ? "connection" : tableChoice && ["features", "agents"].includes(path.split(".")[0]!) ? "subagents" : groupFor(path, !inheritedHook && (options.managedHookPaths?.includes(path) === true || options.trackedPaths?.includes(path) === true));
    ...
    group.settings.push({ path: identity, current, proposed, state, changeKind,
  ...
  const order: ConfigurationReviewGroup["id"][] = ["connection", "subagents", "interrupt", "catalog", "runtime", "integrations", "other"];
