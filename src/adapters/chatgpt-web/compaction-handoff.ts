...
export function structuredCompactionSettlementForNativeTurn(threadId: string, turnId: string): Promise<void> {
  return Promise.all([...structuredCompactionRuns.values()].filter(run => run.active && run.nativeThreadId === threadId && run.nativeTurnId === turnId).map(run => run.settlement)).then(() => undefined);
}
