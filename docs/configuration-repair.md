...
For an active version 10 installation, setup and repair share a pure owned-configuration preparer. Scalar differences, including tracked commented-out routes and an enabled Native V2 setting, can produce a reviewable plan instead of stopping at the old installed expectation. Native preserves newer feature choices; Compatibility V1 proposes its required values. Repair additionally supports the bounded hook repairs described below; uncertain hook identity, source structure and ownership still block changes. Each finding remains separate, and non-setting failures are not rendered as fictitious “not set / unchanged” table rows.

## Diagnostic recovery

The configuration recovery action is labeled **Review fix**. An approved repair reruns Doctor and distinguishes applied changes, required restart, and verified connection checks. Unsupported Doctor findings do not offer Doctor again as their fix: inspect the indicated location or export diagnostic logs. Operation errors retain technical details behind an expandable explanation.

The launcher backend owns the recovery-action policy used by both operation failures and Doctor checks. Structured configuration conflicts offer **Review fix**; a missing route offers **Review setup**. Unknown operation failures offer **Run Doctor** and **Export diagnostic logs**, not a guessed destructive fix or automatic retry. The renderer only translates allowlisted action identifiers into the existing diagnostic, setup-review, repair-review, and sanitized log-export interfaces. Opening a review is not approval to apply it.
...
Reviews put changed settings and findings first. Unchanged settings, including unaffected third-party hooks, are available through **Show unchanged settings**. Activation of a commented value is a change even when its stored value matches. Before/After columns use red for removed values and green for additions, with explicit change labels; exact keys, commands, hashes, source occurrences, and installation baselines remain expandable. Exact file changes use aligned numbered lines, three lines of context, and expandable unchanged gaps. Inline **Hide preview** retains the preview identity and choices; **Refresh preview** invalidates approval, and **Discard preview** removes it. Approval controls are unavailable while collapsed.
...
### Generalized owned-setting repairs

Inspection, repair and review share the owned-scalar definitions in `src/codex-owned-settings.ts`: Responses and WebRTC routes, journal-owned model catalog/provider fields, and Compatibility V1 feature/depth requirements. Native feature values remain user choices, not repair targets. Hook installation, hashing, validation and scalar repair also share their expected command-hook values. Adding a supported owned setting belongs in this shared definition, not in separate Doctor and Repair condition lists.

The source editor handles missing, commented-out and changed owned scalars, including quoted keys, dotted paths, nested inline tables and existing array-table entries. One unambiguous commented assignment is reactivated in place even outside marker sections; competing commented definitions require a source choice. Semantic equivalence is a no-op, preserving numeric representations, whitespace, inline comments, BOMs and line endings. Comments inside strings are never configuration, and table scope remains significant.

An unambiguously identified hook at its journal-recorded position can repair its type, timeout, asynchronous flag and trust value, or reactivate its recorded commented command. A bounded fully commented hook block can be reactivated only when a semantic comparison proves that no unrelated value is enabled or changed. A wholly absent hook still uses reviewed restoration. Changed command identities, duplicate/reordered hooks, unknown extra hook fields and uncertain partial structures remain blocked; scalar editing does not invent array entries or overwrite incompatible subtrees. Every live change still requires the exact preview approval.

Repair requires an active version 10 installation journal. Missing, corrupt, ambiguous, or older ownership evidence is reported as needing attention, not silently adopted. Preview is read-only; restoration requires approval of the exact file plan and updates both ownership records. Other hook events and Native feature choices are preserved. All discovered configuration conflicts are shown together; formatting alone is not a semantic value conflict.
