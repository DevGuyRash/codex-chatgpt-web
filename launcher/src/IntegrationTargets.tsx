...
export function IntegrationTargets({ api, language, disabled }: { api: Pick<LauncherApi, "integrationTargets" | "openIntegrationTarget" | "checkTargetCapabilities" | "setupCore">; language: Language; disabled: boolean }) {
  ...
  const refresh = async () => {
    ...
    setData(result); setSelected(result.selected.id); setHome(result.selected.codexHome);
  };
  useEffect(() => { let mounted = true; void api.integrationTargets().then(result => {
    if (mounted) { setData(result); setSelected(result.selected.id); setHome(result.selected.codexHome); }
  ...
  return <section className="configuration-repair integration-targets" aria-busy={busy}>
    ...
    <details><summary>{copy.create}</summary>
      <label>{copy.home}<input value={home} disabled={disabled || busy} onChange={event => setHome(event.target.value)} /></label>
